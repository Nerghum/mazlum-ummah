...
import { LocalizedText, text } from "@/lib/cms";
import "./style.css";
...
const CrisisStats = ({ cards, numberOverrides }: { cards?: CrisisCard[]; numberOverrides?: CrisisNumberOverrides }) => {
  const t = useTranslations();
  const { locale } = useLocale();

  const CARDS: CrisisCard[] = cards?.length ? cards : [
    {
      ...
      number: text(numberOverrides?.gaza, locale) || t("crisis.gaza.number"),
    ...
    },
    {
      ...
      number: text(numberOverrides?.sudan, locale) || t("crisis.sudan.number"),
    ...
    },
    {
      ...
      number: text(numberOverrides?.middleEast, locale) || t("crisis.middleEast.number"),
      ...
      <Wrapper>
        ...
        <div className="crisis-grid">
          {CARDS.map((card, i) => {
            ...
            return card.link ? (
              <Link key={i} href={card.link} className={`crisis-card crisis-card--${card.gradient}`}>
                {content}
              </Link>
            ) : (
